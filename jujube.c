...
/*Nano-Like Command-Line Text Editor
...
void editorUpdateSyntax(erow *row){
    ...
    while( i < row->rsize){
        ...
        if(mcs_len && mce_len && !in_string){
            ...
            //otherwise, check if at beginning of multi_line comment
...
/*ANSI Escape codes 30-37 give the following colours:
30 - Black
31 - Red
32 - Green
33 - Yellow
...
int editorRowCxToRX(erow *row, int cx){
    ...
    for(j = 0; j < cx; j++){
        if(row->chars[j] == '\t'){
            rx += (JUJUBE_TAB_STOP - 1) - (rx % JUJUBE_TAB_STOP);
        }
        rx++;
    }
    return rx;
...
int editorRowRxToCx(erow *row, int rx){
    ...
    for(cx = 0; cx < row->size; cx++){
        if(row->chars[cx] == '\t')
            cur_rx += (JUJUBE_TAB_STOP - 1) - (cur_rx % JUJUBE_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row){
    ...
    free(row->render);
    row->render = malloc(row->size + tabs*(JUJUBE_TAB_STOP - 1) + 1);

    int idx = 0;
    for(j = 0; j < row->size; j++){
        if(row->chars[j] == '\t'){
            row->render[idx++] = ' ';
            while(idx % JUJUBE_TAB_STOP != 0) row->render[idx++] = ' ';
        } else{
...
void editorProcessKeypress(){
    static int quit_times = JUJUBE_QUIT_TIMES;
    ...
    for(y = 0; y < E.screenrows; y++){
        ...
        if(filerow >= E.numrows){
            if(E.numrows == 0 && y == E.screenrows / 3){
                char welcome[80];
                int welcomelen = snprintf(welcome, sizeof(welcome), "Jujube editor -- version %s", JUJUBE_VERSION);
